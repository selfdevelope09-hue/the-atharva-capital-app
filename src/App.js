...
const AuthProvider = ({ children }) => {
  ...
  return (
    <AuthContext.Provider
      value={{ user, userData, loading, signUp, login, logout, refreshUser }}
    >
      {children}
    </AuthContext.Provider>
  );
...
const ProtectedRoute = ({ children }) => {
  ...
  if (loading)
    return (
      <div style={{ color: T.yellow, textAlign: 'center', marginTop: 80, fontSize: 18 }}>
        Loading...
      </div>
    );
...
const Card = ({ children, style }) => (
  <div style={{ backgroundColor: T.card, borderRadius: 12, padding: 20, ...style }}>
    {children}
  </div>
);
  ...
  return (
    <div style={{ padding: '40px 16px', maxWidth: 1100, margin: '0 auto' }}>
      <div style={{ textAlign: 'center', marginBottom: 60 }}>
        ...
        <p style={{ color: T.text, fontSize: 18, maxWidth: 500, margin: '0 auto 28px' }}>
          Paper Trade Crypto with $0 risk. Master the markets before going live.
        </p>
        {!user && (
          ...
          return (
            <Link key={sym} to={`/trade?symbol=${sym}`} style={{ textDecoration: 'none' }}>
              <Card
                style={{
                  cursor: 'pointer',
                  borderLeft: `3px solid ${chg >= 0 ? T.green : T.red}`
                }}
              >
                <div style={{ color: T.white, fontWeight: 700, fontSize: 15 }}>
                  {sym.replace('USDT', '')}
                </div>
                <div style={{ color: T.white, fontSize: 18, fontWeight: 800, margin: '6px 0' }}>
                  ${d ? parseFloat(d.price).toLocaleString() : '—'}
                </div>
                <div
                  style={{
      ...
      >
        {[
          ...
          <Card key={title}>
            ...
            <div style={{ color: T.white, fontWeight: 700, marginBottom: 6 }}>{title}</div>
            <div style={{ color: T.text, fontSize: 13 }}>{desc}</div>
          </Card>
        ))}
      </div>
    </div>
...
const MarketsScreen = () => {
  ...
  const coins = Object.entries(prices)
    .filter(([sym]) => sym.includes(search.toUpperCase()))
  ...
  return (
    <div style={{ padding: 16, maxWidth: 1000, margin: '0 auto' }}>
      ...
      <div style={{ backgroundColor: T.card, borderRadius: 10, overflow: 'hidden' }}>
        ...
        >
          {['Pair', 'Price', '24h Change', 'Action', ''].map((h) => (
            <span key={h} style={{ color: T.text, fontSize: 12, fontWeight: 600 }}>
              {h}
            </span>
          ))}
        </div>
            ...
            return (
              ...
              >
                <span style={{ color: T.white, fontWeight: 700 }}>
                  {sym.replace('USDT', '')}
                  <span style={{ color: T.text, fontWeight: 400 }}>/USDT</span>
                </span>
                ...
                <span
                  style={{
                    color: chg >= 0 ? T.green : T.red,
                    backgroundColor: chg >= 0 ? 'rgba(2,192,118,0.1)' : 'rgba(246,70,93,0.1)',
                    padding: '3px 10px',
                    borderRadius: 4,
                    fontSize: 13,
...
// -------------------- Trade Screen (with fullscreen chart) --------------------
const TradeScreen = () => {
  const prices = useContext(PriceContext);
  const { user, userData, refreshUser } = useContext(AuthContext);
  ...
  const handleTrade = async () => {
    ...
    if (marginReq > (userData?.virtualBalance || 0))
      return setMsg({ t: 'error', m: `Insufficient balance. Need $${marginReq.toFixed(2)} margin.` });
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        transaction.update(userRef, {
          virtualBalance: increment(-marginReq),
  ...
  return (
    <div style={{ padding: 16, maxWidth: fullscreenChart ? '100%' : 1200, margin: '0 auto' }}>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 16 }}>
        {popularPairs.map((s) => (
          <button
            ...
            onClick={() => setSymbol(s)}
            style={{
              backgroundColor: symbol === s ? T.yellow : T.card2,
      ...
      <div
        style={{
          display: 'grid',
          gridTemplateColumns: fullscreenChart ? '1fr' : '1fr 320px',
          gap: 16
        }}
      >
        <Card style={{ padding: 0, overflow: 'hidden' }}>
          ...
          >
            <span style={{ color: T.white, fontWeight: 700, fontSize: 16 }}>
          ...
          <iframe
            ...
            style={{ width: '100%', height: fullscreenChart ? 600 : 420, border: 'none' }}
            title="Chart"
          />
        </Card>
        {!fullscreenChart && (
          <Card>
            ...
            {userData && (
              <div style={{ color: T.text, fontSize: 12, marginBottom: 10 }}>
                ...
                <span style={{ color: T.white }}>
                  ${parseFloat(userData.virtualBalance || 0).toFixed(2)} USDT
                </span>
              </div>
            )}
            {orderType === 'Limit' && (
              <div style={{ marginBottom: 10 }}>
                <label
                  style={{ color: T.text, fontSize: 12, display: 'block', marginBottom: 4 }}
                >
                  Limit Price
                </label>
                <Input
            ...
            <div style={{ marginBottom: 14 }}>
              <label
                style={{ color: T.text, fontSize: 12, display: 'block', marginBottom: 6 }}
              >
                Leverage:{' '}
                <span style={{ color: T.yellow, fontWeight: 700 }}>{leverage}x</span>
              </label>
              <input
                type="range"
                min={1}
            ...
            >
              <div>
                <label
                  style={{ color: T.text, fontSize: 11, display: 'block', marginBottom: 4 }}
                >
                  Take Profit
                </label>
                <Input
...
// -------------------- Dashboard (with PnL Chart & Order History) --------------------
    ...
    try {
      ...
      await runTransaction(db, async (transaction) => {
        ...
        transaction.update(userRef, {
          ...
          closedPositions: [...closedPositions, closedPosition]
        });
      });
      await refreshUser();
      setCloseMsg({
        t: finalPnl >= 0 ? 'success' : 'error',
        m: `Position closed. PnL: ${finalPnl >= 0 ? '+' : ''}$${finalPnl.toFixed(2)} (0.05% fee deducted)`
      });
    } catch (e) {
      setCloseMsg({ t: 'error', m: e.message });
    }
  };

  // Prepare equity curve data (combine closed positions PnL for simplicity)
  const equityData = (userData?.closedPositions || []).map((pos, idx) => ({
    name: `Trade ${idx + 1}`,
    pnl: pos.realizedPnl
  }));
  ...
  return (
    <div style={{ padding: 16, maxWidth: 1200, margin: '0 auto' }}>
      ...
      >
        ...
        <Card>
          <div style={{ color: T.text, fontSize: 12, marginBottom: 6 }}>Unrealized PnL</div>
          <div
            style={{
        ...
        <Card style={{ marginBottom: 24 }}>
          ...
          <ResponsiveContainer width="100%" height={200}>
            <LineChart data={equityData}>
              ...
              <YAxis stroke={T.text} />
              <Tooltip
                contentStyle={{ backgroundColor: T.card, border: 'none' }}
      ...
      {userData?.closedPositions?.length > 0 ? (
        userData.closedPositions
          .slice()
          .reverse()
          .map((pos, idx) => (
            <Card key={idx} style={{ marginBottom: 8, opacity: 0.9 }}>
              ...
              >
                ...
                <span
                  style={{
                    color: pos.realizedPnl >= 0 ? T.green : T.red,
                    fontWeight: 600
                  }}
                >
              ...
              <div style={{ color: T.text, fontSize: 12, marginTop: 4 }}>
                Entry ${pos.entryPrice.toFixed(2)} → Exit ${pos.exitPrice.toFixed(2)} | Size $
                {pos.totalSize.toFixed(2)} | Leverage {pos.leverage}x
              </div>
            </Card>
          ))
      ) : (
  ...
  return (
    <div style={{ padding: 16, maxWidth: 600, margin: '0 auto' }}>
      ...
      <Card style={{ marginBottom: 20 }}>
        ...
        <div style={{ color: T.text, fontSize: 12, marginTop: 8 }}>
          Paper trading account • No real funds
        </div>
      </Card>

      <Card style={{ marginBottom: 20, border: `1px dashed ${T.border}` }}>
        <h3 style={{ color: T.white, marginTop: 0, marginBottom: 12, fontSize: 16 }}>
          Top Up Virtual Funds
        </h3>
        <p style={{ color: T.text, fontSize: 13, marginBottom: 16 }}>
          Need more capital to test strategies? Add virtual USDT instantly.
        </p>
        <div
          style={{
            display: 'grid',
            gridTemplateColumns: '1fr 1fr',
            gap: 12,
            marginBottom: msg ? 16 : 0
          }}
        >
          <Btn
            onClick={() => handleTopUp(1000)}
            disabled={topUpLoading}
            style={{ backgroundColor: T.card2, color: T.white, border: `1px solid ${T.border}` }}
          >
            + $1,000
          </Btn>
          <Btn
      ...
      <Card>
        ...
        {(!userData?.portfolio || userData.portfolio.length === 0) && (
          <div style={{ color: T.text, textAlign: 'center', padding: '16px 0', fontSize: 13 }}>
            All funds are in USDT. Start trading to open positions.
          </div>
        )}
      </Card>
    </div>
      ...
      <Card style={{ padding: 0, overflow: 'hidden' }}>
        ...
        {loading ? (
          ...
          users.map((u, idx) => (
            ...
            >
              <span style={{ color: idx < 3 ? T.yellow : T.white, fontWeight: 600 }}>
                {idx + 1}
              </span>
              <span style={{ color: T.white }}>{u.name || 'Anonymous'}</span>
              <span style={{ color: T.green, fontWeight: 600, textAlign: 'right' }}>
                ${u.virtualBalance?.toLocaleString('en-US', { minimumFractionDigits: 2 })}
              </span>
            </div>
          ))
        )}
  ...
  return (
    ...
    >
      <Card style={{ width: '100%', maxWidth: 380 }}>
        ...
        <div style={{ color: T.text, textAlign: 'center', marginBottom: 28, fontSize: 14 }}>
          Welcome back
        </div>
        <label style={{ color: T.text, fontSize: 12, display: 'block', marginBottom: 4 }}>
          Email
  ...
  return (
    ...
    >
      <Card style={{ width: '100%', maxWidth: 380 }}>
        ...
        <div style={{ color: T.text, textAlign: 'center', marginBottom: 28, fontSize: 14 }}>
          Get $10,000 virtual USDT to trade with
        </div>
        <label style={{ color: T.text, fontSize: 12, display: 'block', marginBottom: 4 }}>
          Your Name
        </label>
