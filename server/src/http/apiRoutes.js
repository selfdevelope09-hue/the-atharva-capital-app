...
router.get('/api/chat/community/unread', async (req, res) => {
  try {
    const uid = req.user.uid;
    const { rows: stateRows } = await getPool().query(
      `select last_seen_at from community_read_state where uid = $1`,
      [uid]
    );
    ...
    const { rows: countRows } = await getPool().query(
      `select count(*)::int as n from community_messages
       where created_at > $2 and from_uid <> $1`,
      [uid, lastSeen]
    );
...
router.get('/api/chat/community/messages', async (req, res) => {
  try {
    const lim = Math.min(200, Math.max(1, parseInt(String(req.query.limit || '100'), 10) || 100));
    const beforeRaw = String(req.query.before || '').trim();
    let q = `select * from community_messages`;
    const params = [];
    if (beforeRaw) {
      q += ` where created_at < $1::timestamptz`;
...
router.post('/api/chat/community/mark-read', async (req, res) => {
  try {
    const actorUid = await resolveChatActor(req);
    const fromName = String(req.body?.fromName || 'Trader').trim() || 'Trader';
    await getPool().query(
      `insert into community_read_state (uid, from_name, last_seen_at)
       values ($1, $2, now())
       on conflict (uid) do update set from_name = excluded.from_name, last_seen_at = now()`,
      [actorUid, fromName]
    );
    await getPool().query(
      `insert into community_message_reads (message_id, uid, from_name, read_at)
       select id, $1, $2, now() from community_messages
...
router.post('/api/chat/community/send', async (req, res) => {
  try {
    const actorUid = await resolveChatActor(req);
    ...
    await getPool().query(
      `insert into community_messages (id, from_uid, from_name, text, image_url, file_url, file_name, media_kind)
    ...
    res.json({ ok: true, message: messageRowToClient(savedRows[0]) });
  } catch (e) {
    const code = e.statusCode || 500;
    res.status(code).json({ ok: false, error: e.message });
  }
...
router.post('/api/chat/community/delete', async (req, res) => {
  try {
